...
import { PaymentStatus } from "@/types/admin-order";

interface OrderPaymentDropdownProps {
  value: "ALL" | PaymentStatus;
  onChange: (val: "ALL" | PaymentStatus) => void;
}

const PAYMENT_OPTIONS: { value: "ALL" | PaymentStatus; label: string; shortLabel: string }[] = [
  { value: "ALL", label: "All Payments", shortLabel: "All Payments" },
  { value: "COMPLETED", label: "Paid (Completed)", shortLabel: "Paid" },
  { value: "PENDING", label: "Pending Payment", shortLabel: "Pending" },
  { value: "FAILED", label: "Failed", shortLabel: "Failed" },
  { value: "REFUNDED", label: "Refunded", shortLabel: "Refunded" },
];

export const OrderPaymentDropdown: React.FC<OrderPaymentDropdownProps> = ({ value, onChange }) => {
  ...
  const currentOption = PAYMENT_OPTIONS.find((o) => o.value === value) || PAYMENT_OPTIONS[0];
    ...
    <div ref={dropdownRef} className="relative w-full">
      ...
      >
        <div className="flex items-center gap-1.5 min-w-0 truncate">
          <CreditCard className={`h-3.5 w-3.5 shrink-0 ${isFiltered ? "text-orange-600" : "text-slate-400"}`} />
      ...
      {isOpen && (
        <>
          ...
          <div
            role="listbox"
            className="absolute top-full left-0 sm:left-auto sm:right-0 mt-1.5 w-52 max-w-[calc(100vw-2rem)] bg-white rounded-2xl shadow-2xl border border-slate-200/90 z-50 overflow-hidden p-1.5 space-y-0.5 text-xs animate-fade-in"
          >
...
export default OrderPaymentDropdown;
